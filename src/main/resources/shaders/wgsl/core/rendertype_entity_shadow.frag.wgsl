// Entity shadow fragment shader
// Converted from rendertype_entity_shadow.fsh
...
fn main_fs(in: FragmentInput) -> @location(0) vec4<f32> {
    let clamped_uv = clamp(in.tex_coord0, vec2<f32>(0.0), vec2<f32>(1.0));
    var color = textureSample(texture0, sampler0, clamped_uv);
    color = color * in.vertex_color * dynamic_transforms.ColorModulator;
