...
fn apply_fog(in_color: vec4<f32>, spherical_dist: f32, fog_color: vec4<f32>, fog_end: f32) -> vec4<f32> {
    let fog_value = linear_fog_value(spherical_dist, 0.0, fog_end);
...
fn main_fs(in: FragmentInput) -> @location(0) vec4<f32> {
    return apply_fog(dynamic_transforms.ColorModulator, in.spherical_vertex_distance, fog.FogColor, fog.FogSkyEnd);
}
