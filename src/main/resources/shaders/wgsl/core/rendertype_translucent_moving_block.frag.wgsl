// Translucent moving block fragment shader
// Converted from rendertype_translucent_moving_block.fsh
...
fn main_fs(in: FragmentInput) -> @location(0) vec4<f32> {
    var color = textureSample(texture0, sampler0, in.tex_coord0) * in.vertex_color;
    color = color * dynamic_transforms.ColorModulator;
