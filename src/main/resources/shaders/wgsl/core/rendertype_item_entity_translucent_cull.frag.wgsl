// Item entity translucent cull fragment shader
// Converted from rendertype_item_entity_translucent_cull.fsh
...
fn main_fs(in: FragmentInput) -> @location(0) vec4<f32> {
    var color = textureSample(texture0, sampler0, in.tex_coord0) * in.vertex_color * dynamic_transforms.ColorModulator;
    if (color.a < 0.1) {
        discard;
    }
