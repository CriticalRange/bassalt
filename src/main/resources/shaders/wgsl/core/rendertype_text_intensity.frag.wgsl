...
// Converted from rendertype_text_intensity.fsh
// Uses red channel for intensity (monochrome text)
...
fn main_fs(in: FragmentInput) -> @location(0) vec4<f32> {
    ...
    var color = vec4<f32>(tex_sample.r, tex_sample.r, tex_sample.r, tex_sample.r) * in.vertex_color * dynamic_transforms.ColorModulator;
    if (color.a < 0.1) {
        discard;
    }
