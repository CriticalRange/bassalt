...
fn main_fs(in: FragmentInput) -> @location(0) vec4<f32> {
    return apply_fog(dynamic_transforms.ColorModulator, in.spherical_vertex_distance, in.cylindrical_vertex_distance, fog.FogColor, fog.FogEnvironmentalStart, fog.FogEnvironmentalEnd, fog.FogRenderDistanceStart, fog.FogRenderDistanceEnd);
}
