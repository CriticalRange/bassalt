// Entity decal fragment shader
// Converted from rendertype_entity_decal.fsh
...
fn main_fs(in: FragmentInput) -> @location(0) vec4<f32> {
    var color = textureSample(texture0, sampler0, in.tex_coord0);
    if (color.a < 0.1) {
        discard;
    }
    // No overlay mixing in simplified version
